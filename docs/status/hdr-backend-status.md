...
| macOS | Flutter texture tone-map | 探测绑定窗口实际所在 `NSScreen`，并监听跨屏/显示参数变化；`configureOutput` 仍 fail-closed，原生 layer 尚未接入。重建判断只比较输出载体，SDR/HDR tone-map 共用 Texture，因此元数据变化只更新 mpv 参数；保持原有相关视频切换逻辑时连续切换复测无黑屏 | 使用窗口内 `CAMetalLayer`，设置 EDR 色彩空间/像素格式并验证外接 SDR 回退 |

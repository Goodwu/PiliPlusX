...
"$ssh_wrapper" --ssh-opt '-oClearAllForwardings=yes' "$remote_host" -- "$remote_project" "$version" "$build_number" "$keep_permission" >"$unsigned" <<'REMOTE'
set -euo pipefail
src=$1
version=$2
build_number=$3
...
cd "$src"
flutter build hap --debug --no-codesign --build-name "$version" --build-number "$build_number" >&2
